...
## Acceptance Criteria
- [ ] Given an official post, when it lands, then the engine computes a suggested storyline match with
      a confidence (similarity to expectation + hashtags) and surfaces it in the controller prompt
      ("does this address #X? Y/N").
- [ ] Given controller confirmations over time, when suggestions are scored, then the engine tracks
      **rolling precision** of its suggestions within the exercise (telemetry-backed).
- [ ] Given precision holds above a threshold over a sustained window, when the condition is met, then
      the console **offers** an opt-in to auto-confirm matches above a confidence cutoff — it is
      **never** auto-enabled by the engine.
- [ ] Given auto-match is opted in, when the engine auto-confirms, then every auto-match is still
      logged and reversible, and the miss-safe default (story 02) still applies to anything below the
      cutoff.
- [ ] **Telemetry (XC-004):** suggestions, confirmations, precision, and any auto-match opt-in state
