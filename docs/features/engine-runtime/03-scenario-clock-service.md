...
## Acceptance Criteria
- [ ] **StartEx + monotonic tick.** Given an exercise starts, When StartEx fires, Then the clock begins
...
- [ ] **Freeze stops the clock (COR-052 / CTL-023).** Given a Freeze-world command, When the scenario
...
- [ ] **Discrete time-jump (COR-051 / CTL-015).** Given a Director time-jump of N scenario minutes, When
...
- [ ] **Swappable provider (COR-050).** Given the clock sits behind `IExerciseClock`, Then the native
  ...
  `AddEngineGeneration`.
- [ ] **Replaces the hand-cranked `IScenarioClock`.** Given the engine reads scenario minutes only
...
- [ ] **Scenario time only (COR-053).** Given the loop's timers and the Delayed-auto countdown, Then
...
- [ ] **Isolation (COR-001).** Given a per-exercise clock, When exercise A is frozen or jumped, Then it
