...
**Requirements:** console UI architecture (D5)  ·  **Design decisions:** D5-016, D5-017, D5-019  ·  **Issue:** #9
...
## Acceptance Criteria
- [ ] Given the console, when it renders, then a 56px right-edge toolstrip shows the available tools
      as icons (FontAwesome) with accessible labels, and continuous-watch surfaces occupy permanent
      rail/column space rather than the toolstrip.
...
## Technical Notes
Staff world (COBRA). Owns the console layout shell + toolstrip/flyout primitives under
`features/controller/components/shell/`. Continuous-watch vs consult-on-demand is a per-tool config,
not per-instance logic. FontAwesome icons; MUI 9 `sx`-only. See implementation.md (story 01).
