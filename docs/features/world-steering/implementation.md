...
> implements `IExerciseClock` and is installed via the SHIPPED `setExerciseClock()` on Freeze (and
> restored via `resetExerciseClock()`/re-`setExerciseClock()` with a running implementation on
> Resume). This is the mock stand-in for story-01's native pause-aware clock provider (deferred) /
