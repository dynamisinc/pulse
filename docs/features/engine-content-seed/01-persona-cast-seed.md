...
Bad-actor / impersonator personas (SOC-052 lookalike training pair) — see `feature.md` Design notes for
why they are deliberately excluded this pass. Avatar images (COR-024). Believable derived state beyond a
fixed placeholder (varied follower counts / backdated join dates are COR-021's job, not this story's).
Any change to `PersonaEndpoints.cs` (`GET /api/personas` stays read-only and unmodified; it will simply
start returning real rows once this story lands).
...
**The six starter personas' dossiers** (handle → `PersonaType` → voice-note gist), matching the shipped
frontend mock exactly (same handles/names/kind/verified so `GET /api/personas` and this seed never
disagree once persona-management's real authoring lands and can reconcile against these rows):
...
`ReactionLoopHostTests`' harness): seeding a fresh exercise creates exactly six rows with the exact
handles/kind/verified table above; seeding twice creates zero additional rows and returns the same ids;
seeding exercise A never creates or reads rows scoped to exercise B (extends the standing isolation
