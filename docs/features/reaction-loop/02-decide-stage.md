...
## Acceptance Criteria
- [ ] Given observed triggers + storyline state, when the decide stage runs, then it emits a
...
- [ ] Given rate caps / quiet floors (ADP-011), when intent is formed, then it respects
      `maxEnginePostsPerMinute` (won't request a burst that breaches the cap) and honors
      `minBelievableActivity` (drives ambient intent when below floor).
- [ ] Given the autonomy level, when intent is formed, then it is annotated with the level
      (Suggest / Delayed-auto) so the review stage (story 03) routes correctly.
- [ ] Given the dial target, when set, then the intent's intensity/volume is modulated toward it
...
- [ ] **Telemetry (XC-004):** the decision emits an `engine.decided` event (intent, autonomy,
      rate-cap state, trigger) — staff-only (XC-002).
