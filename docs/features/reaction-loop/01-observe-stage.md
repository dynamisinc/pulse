...
> **Delivered:** `ReactionLoop/Services/ObserveStage.Observe` — scenario-time inaction triggers (silence
> window elapsed, read from storyline state advanced by `Storyline.Tick`; freeze halts, time-jump advances)
> + addressing candidates surfaced (matching is response-reaction's; never treated as silence, ADP-002a).
> Tests: `ObserveStageTests`. (`engine.observed` telemetry emission waits on #173 / E1 XC-004 base.)
...
## Acceptance Criteria
- [x] Given active storylines, when the loop observes, then it collects: recent participant/world
      actions relevant to each storyline, elapsed inaction timers, and the current dial target.
- [x] Given a storyline's `responseWindowMin`, when it elapses in **scenario time** without a
      qualifying response, then an inaction trigger is raised for that storyline (the silence signal
      silence-escalation consumes).
- [x] Given a scenario-time **freeze** (CTL-023) or **time-jump** (CTL-015), when it occurs, then
      inaction timers stop/advance accordingly — windows do not elapse while the world is frozen.
- [x] Given an official response or off-platform marker (CTL-026), when observed, then it is surfaced
      to the decide stage as a candidate storyline-addressing event (matching happens in
      response-reaction).
- [x] **Scenario time (COR-050/051):** all timers/windows are scenario minutes, driven by the E1
      clock provider; wall-clock is telemetry-only.
- [x] Observation is **staff/backend** and exercise-scoped (COR-001, XC-002).
