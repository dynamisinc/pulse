...
> `exercise-isolation/01`'s filter as realized by `backend-host/03`), authored in parallel by a
...
- The exercise-scoping global query filter — `exercise-isolation/01-exercise-scoped-queries` (Not Started; the requirement, COR-001, is E1's), realized in real SQL by `backend-host/03-exercise-isolation-filter` **[Tier-2]**. Every query/persist/group-join in this feature inherits it; no story here re-implements scoping.
- `IExerciseContext` (`backend-host/03`) — per-request scope resolution (hostname/session-token). All four stories read the request's resolved scope from it; none accepts a client-supplied `exerciseId` as a scoping parameter.
- `backend-host/04-telemetry-sink` — the real `POST /telemetry` ingest behind the locked v0 envelope. Story 02's server-side emission targets this sink.
...
|-------|-------|----------------|------------|---------------|------|--------|
| 01 Feed & thread read | fullstack | `Pulse.WebApi/Features/Social/{FeedEndpoints.cs,ThreadEndpoints.cs,ParticipantPostDto.cs,PostReadService.cs}` (+ xUnit) | B0 (`backend-host/01,02`; filter via `backend-host/03`) | 02, 03, 04 | 1 | M |
| 02 Post write | fullstack | `Pulse.WebApi/Features/Social/{PostWriteEndpoints.cs,PostIngestService.cs,PostSanitizer.cs}` (+ xUnit) | B0 (as above) + `backend-host/04` (telemetry sink); contract-first `IFeedBroadcaster` interface with 03 (see note*) | 01, 03*, 04 | 1 | L |
