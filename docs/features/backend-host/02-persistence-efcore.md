...
## Acceptance Criteria
- [ ] Given `Pulse.WebApi`, when its EF Core model is built, then `PulseDbContext` exposes
...
- [ ] Given the `Post` entity, when its schema is defined, then it reserves nullable extension columns
...
- [ ] Given the `TelemetryEvent` entity, when its columns are compared field-by-field to the locked v0
...
- [ ] Given a clean database, when the initial EF Core migration is applied, then it succeeds against an
...
- [ ] Given `Program.cs` (owned by story 01), when this story registers persistence, then it exposes an
...
- [ ] **Isolation (COR-001/XC-001) — Tier-2, human sign-off required:** every scoped entity's
