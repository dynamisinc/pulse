...
| 03 Persona handle uniqueness | A post-B1 hardening item under §3 principle 6 (*"do not treat the first migration as final"*), not part of the original B0 wave plan: extends story 02's `Persona` model block with `HasMaxLength(256)` on `Handle` (nvarchar(max) is not index-key eligible) + `HasIndex(ExerciseId, Handle).IsUnique()`, and ships migration `PersonaHandleUniqueIndex`. The migration's `Up` opens with a hand-written pre-flight that THROWs a diagnostic on pre-existing duplicates rather than de-duplicating — see the story's Technical Notes for why, and for the read-only pre-flight query to run against a populated environment first. | `src/Pulse.WebApi/Data/PulseDbContext.cs`, `Data/Entities/Persona.cs`, `Data/Migrations/20260725120413_PersonaHandleUniqueIndex.cs`; `src/Pulse.WebApi.Tests/Data/PersonaHandleUniqueness*Tests.cs` | The enforced `(ExerciseId, Handle)` invariant `PersonaCastSeeder`, `EngineReviewService.ResolvePersonaHandlesAsync`, and #342's `OpsPersonaResolver` already assume; the resolved §7 Q3 policy `persona-management/03` + `persona-operation/05` validate against |
...
| `PulseDbContext.OnModelCreating` | `src/Pulse.WebApi/Data/PulseDbContext.cs` | Not a wave-fan-out seam (a small number of serial extenders, not a parallel fan-out) — story 02 **creates** it; `exercise-isolation/01` (a different feature, serially after 02) directly **extends** it with the read-side global query filter, and story 03 later extends the `Persona` entity block with the `(ExerciseId, Handle)` unique index (so this row's original *"only one known future extender"* framing no longer holds — expect further per-entity constraint additions here as hardening items, each with its own migration), the same "create then extend" pattern already used for `core/exerciseContext.tsx` (`exercise-isolation/10` creates → `exercise-isolation/04` extends). Documented here so a future builder does not stand up a second `DbContext`. Unlike the two rows above, this one *is* a normal serial story dependency (declared in `exercise-isolation/01`'s own Depends-on), not an orchestrator-only edit — listed here for visibility since it is the seam this whole feature exists to hand off. |
