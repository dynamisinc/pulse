...
**Feature:** Login & UAT go-live  ·  **Epic:** E1  ·  **Phase:** 1  ·  **Status:** In Progress (code portion in a follow-up Wave-4 PR this session; runbook execution human-gated)
...
> **Status update (this feature's close-out):** the **code portion** (bicep param threading + workflow
> secrets) is landing in a follow-up Wave-4 PR this session; the **runbook execution** below (setting the
> new GitHub secrets, running the bootstrap endpoint, flipping `VITE_USE_MOCK_DATA`, redeploying the
> frontend) is a **human-gated** step and is not run by an agent. The Wave-4 PR refreshes the AC checkboxes
> and the exact secret names below to their as-built values.
