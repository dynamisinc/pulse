...
- [x] Audience magnitude is defined (band from E1 COR-020/SOC-054, evolving with activity) and this
      ...
      Magnitude itself is the E1 band-derived `Persona.followerCount` (`personas/seedCast.ts`) in
      mock mode, and the real, backend-persisted `Persona.AudienceMagnitude` (story 06/#369) in live
      mode; "evolving with activity" is an E8 concern that consumes this module, not a change to it.
- [ ] Counts are exercise-scoped (COR-001). **UNTICKED — no test covers this yet.** See "AC4 status"
      below; do not tick this box until a test exists.
