...
## Acceptance Criteria
- [ ] Given the feed's mock adapter, when `feedService.resolveFeed()` resolves, then it reads from a
...
- [ ] Given a post is appended via `postStore.appendPost(post)`, when `useFeed()` re-derives (via its
...
- [ ] The feed's existing `aria-live="polite"` `<ul>` region (`Feed.tsx`) announces the arrival — this
...
- [ ] `postStore` is exercise-scoped **by construction, stamping-only**: appended `Post`s already carry
...
- [ ] `postStore` exposes `resetForTests()` so tests can restore the seeded baseline between cases
