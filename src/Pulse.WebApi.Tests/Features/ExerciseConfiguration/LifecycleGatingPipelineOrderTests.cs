...
/// failure COR-032 AC3 forbids; scope never resolved at all → <c>401</c> from the endpoint's own fail-closed
/// guard. A companion test pins the <c>live</c> case at <c>200</c> so a blanket-refusing pipeline (or a
/// resolution break) cannot make the <c>403</c> pass for the wrong reason.
/// </para>
/// <para>
...
public sealed class LifecycleGatingPipelineOrderTests
{
    ...
    {
        var host = await SeedExerciseAsync("archived", withPost: true);

        await using var factory = new LifecycleGatingProbeFactory(_fixture.ConnectionString!);
        using var client = CreateClientFor(factory, host);
        ...
        response.StatusCode.Should().Be(
            ...
            + "participant an archived world's posts. A 401 means the host never resolved to an exercise at all");
    }
    ...
    {
        var host = await SeedExerciseAsync("live", withPost: true);
    ...
    /// <c>UseExerciseResolution()</c> can map a request to it) and, optionally, one visible post — so a
    /// wrongly-served feed is provably non-empty rather than an empty-but-200.
    /// </summary>
    /// <returns>The provisioned hostname to address the exercise by.</returns>
    private async Task<string> SeedExerciseAsync(string status, bool withPost)
    {
    ...
    /// </summary>
    private static HttpClient CreateClientFor(WebApplicationFactory<Program> factory, string host)
    {
    ...
    /// pipeline or the exercise context would make the ordering it exists to prove unobservable.
    /// </summary>
