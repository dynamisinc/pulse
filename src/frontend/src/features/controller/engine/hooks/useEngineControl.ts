 ...
 *
 * `modeSettledCount` (autonomy-safety story 06, Gate-1 CR-101). A monotonic
 * counter bumped in BOTH the `.then` and `.catch` of the live kill-switch/
 * restore POST — i.e. once the request SETTLES, never on the synchronous
 * optimistic flip above it. `<EngineControlBar>` watches this (not raw
 * `mode`) to decide when to refetch `useEngineSettings()`'s snapshot: the
 * optimistic flip and the POST are issued in the same call, so a watcher keyed
 * on `mode` fires a settings GET while the kill-switch POST is still
 * in-flight — a race the GET is heavily favoured to win (one filter vs. the
 * POST's mutation + validation + a telemetry write), which is worse than no
 * refetch at all: it applies a pre-trip snapshot as authoritative and nothing
 * ever corrects it, since neither `mode` nor `degraded` changes again.
 * `modeSettledCount` is a pure signal — it carries no information itself,
 * every consumer must still read `mode`/`degraded`/`effective` for the actual
 * state; it exists ONLY to say "a live request just concluded, whatever it
 * was". Untouched in mock mode (there is no live POST to settle).
 */
...
export interface UseEngineControlResult {
  ...
  /**
   * Bumped once per live kill-switch/restore POST SETTLING (Gate-1 CR-101) —
   * a pure signal, never itself the state; watch it (not raw `mode`) to know
   * when a settings refetch is safe to fire without racing the POST. Never
  ...
  const setMode = useCallback(
    (mode: EngineMode, options?: SetEngineModeOptions) => {
      ...
      liveSetMode(mode, { actingHumanId: identity.actingHumanId, timeZone })
        .then(() => {
          // Settled successfully. Bump the settle signal (Gate-1 CR-101) so
          // `<EngineControlBar>`'s engine-settings refetch fires AFTER the
          // POST has actually concluded, not on this function's earlier
        ...
        .catch(() => {
          ...
          // Bumped unconditionally (Gate-1 CR-101) — even a stale/superseded
          // rejection means A live request just settled, which is still a
          // valid moment to refetch engine settings.
          setFor(exerciseId, {
            ...live,
            mode: revertedMode,
